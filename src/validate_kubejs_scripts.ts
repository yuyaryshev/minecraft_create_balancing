        ...
        shaped: (result: string, shape: string[], ingredients: any) => {
            ...
            for (const row of shape) {
                if (row.length !== rowLength) {
                    console.trace(`CODE00000082 Shaped recipe ${result} has invalid shape!`);
                }
            }
            ...
            for (let k of allKeys) {
                if (!ingredients[k]) {
                    console.trace(`CODE00000083 Key '${k}' is not resolved in ingredients of a shaped recipe ${result}!`);
                }
            }

            if (typeof ingredients !== "object") {
                console.trace(`CODE00000084 Ingredients should be an object!`);
            }

            for (const k in ingredients) {
                const v = ingredients[k];
                if (!allKeys.has(k)) {
                    console.trace(`CODE00000091 Ingredient '${k}' not present in shape of a shaped recipe ${result}!`);
                }
                if (typeof v === "string") {
                ...
                .map(
                    (n) =>
                        `function makeEventValidator_${eventNameToFuncName(
                            n,
                        )}() {\n\tconsole.trace(\`CODE00000092 Validator for ${n} not implemented!\`);\n\treturn {};\n};\n\n`,
                )
                .join(""),
        );
    }
